...
def move(current_position: Tuple, instruction: Tuple) -> List[Tuple]:
    x, y = current_position
    direction, size = instruction

    if direction == "U":
        new_position = [(x, y + i) for i in range(1, size + 1)]
    elif direction == "D":
        new_position = [(x, y - i) for i in range(1, size + 1)]
    elif direction == "R":
        new_position = [(x + i, y) for i in range(1, size + 1)]
    elif direction == "L":
        new_position = [(x - i, y) for i in range(1, size + 1)]
    else:
        raise Exception("Uknown direction {}" % direction)

    return new_position


def calculate_path(instructions: List[Tuple]) -> List[Tuple]:

    initial_position = (0, 0)
    trail = [initial_position]

    for instruction in instructions:
        current_position = trail[-1]
        new_position = move(current_position, instruction)
        trail += new_position

    return trail


def find_cross_points(wire1: str, wire2: str) -> Set[Tuple]:
    w1 = format(wire1)
    w2 = format(wire2)

    path1 = calculate_path(w1)
    path2 = calculate_path(w2)

    cross_points = set(path1) & set(path2)
    return cross_points


def smallest_manhattan_d(points: Set) -> int:
    distances = points.remove((0, 0))
...
assert smallest_manhattan_d(cross_points) == 159

cross_points = find_cross_points("R8,U5,L5,D3", "U7,R6,D4,L4")
assert smallest_manhattan_d(cross_points) == 6

cross_points = find_cross_points(WIRE1, WIRE2)
print(smallest_manhattan_d(cross_points))

# Part II
# Number of Steps

# For all intersections, calculate the number of steps to get there for both players


path1 = calculate_path(format(WIRE1))
path2 = calculate_path(format(WIRE2))

n_steps = [(cp, path1.index(cp) + path2.index(cp)) for cp in cross_points]

min_n_steps = min([n_step for _, n_step in n_steps])

nearest_cp = [(cp, n_step) for cp, n_step in n_steps if n_step == min_n_steps]
